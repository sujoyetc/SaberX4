...
uint8_t bytes0[2028], bytes1[2028], bytes2[2028], bytes3[2028];
...
void test_POLVECp2BS(uint8_t *bytes, uint16_t data[SABER_K][SABER_N]){
	clock_S = cpucycles();
	uint32_t i,j;
...
void test_POLVECp2BSX4(uint8_t *bytes0, uint8_t *bytes1, uint8_t *bytes2, uint8_t *bytes3, 
	...
	//uint64_t tmp_data[SABER_K*SABER_N][4];
	...
	for(k=0;k<SABER_K;++k){
		for(n=0;n<SABER_N;++n){
			...
			//tmp_data[k*SABER_N+n][0] = data0[k][n];
			//tmp_data[k*SABER_N+n][1] = data1[k][n];
			//tmp_data[k*SABER_N+n][2] = data2[k][n];
			//tmp_data[k*SABER_N+n][3] = data3[k][n];
			
			//avx_data[k*SABER_N+n] = _mm256_loadu_si256((__m256i const *)(tmp_data[k*SABER_N+n]));
		}
	}
	clock_E0 = cpucycles();
...
int main()
{
	...
	int i;
	for(i=0;i<10000;i++){
		test_POLVECp2BS(bytes0,data0);
		test_POLVECp2BS(bytes1,data1);
		test_POLVECp2BS(bytes2,data2);
		test_POLVECp2BS(bytes3,data3);
		test_POLVECp2BSX4(bytes0,bytes1,bytes2,bytes3,data0,data1,data2,data3);
	}
