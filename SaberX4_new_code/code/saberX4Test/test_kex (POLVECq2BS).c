...
void test_POLVECq2BS(uint8_t *bytes, uint16_t data[SABER_K][SABER_N]){
	clock_S = cpucycles();
...
void test_POLVECq2BSX4(uint8_t *bytes0, uint8_t *bytes1, uint8_t *bytes2, uint8_t *bytes3, 
...
int main()
{
	...
	int i;
	for(i=0;i<10000;i++){
		test_POLVECq2BS(bytes0,data0);
		test_POLVECq2BS(bytes1,data1);
		test_POLVECq2BS(bytes2,data2);
		test_POLVECq2BS(bytes3,data3);
		test_POLVECq2BSX4(bytes0,bytes1,bytes2,bytes3,data0,data1,data2,data3);
	}
