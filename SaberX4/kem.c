						...
						unsigned char *pk3, unsigned char *sk3)
{
  ...
  indcpa_kem_keypair(pk0,sk0, pk1,sk1, pk2,sk2, pk3,sk3);  // sk[0:SABER_INDCPA_SECRETKEYBYTES-1] <-- sk
  ...
  //randombytes(sk+SABER_SECRETKEYBYTES-SABER_KEYBYTES , SABER_KEYBYTES );    // Remaining part of sk contains a pseudo-random number. 
	...
	randombytes0(sk0+SABER_SECRETKEYBYTES-SABER_KEYBYTES , SABER_KEYBYTES );
	randombytes1(sk1+SABER_SECRETKEYBYTES-SABER_KEYBYTES , SABER_KEYBYTES );
	randombytes2(sk2+SABER_SECRETKEYBYTES-SABER_KEYBYTES , SABER_KEYBYTES );
	randombytes3(sk3+SABER_SECRETKEYBYTES-SABER_KEYBYTES , SABER_KEYBYTES );
									...
									unsigned char *c3, unsigned char *k3, const unsigned char *pk3)
{
  ...
  //randombytes(buf, 32); 
	randombytes0(buf0, 32); randombytes1(buf1, 32); randombytes2(buf2, 32); randombytes3(buf3, 32);
